        ...
        for (var y = x + 1; y < options.types.length; y += 1) {
            if (options.types[x].type.capitalizeFirstLetter() ===
                ...
                throw new Error(("Invalid parameter 'options.types' supplied to Seatchart. " +
                                ...
                                'Types are case insensitive.').format(options.types[x].type, options.types[y].type));
            }
        }
    }
    ...
    */
    ...
    var updateTotal = function updateTotal() {
        ...
        if (cartItemsCounter !== undefined) {
            cartItemsCounter.textContent = '{0}'.format(cartTable.childNodes.length)+' out of '+ passengersNum;;
        }
    };
     ...
     * @private
     */
    var columnName = function rowName(index, disabled, disabledCount) {
        if (!disabled) {
     ...
     * @returns {string} Column name. Return null or undefined if empty.
     */
    var rowName = function columnName(index, disabled, disabledCount) {
        if (!disabled) {
    ...
    this.onChange = function() {
      updateCartObject();
      console.log("changed", this.getCart());
      if(this.getCart().selected.length === passengersNum){
        var available = [...document.getElementsByClassName("sc-seat available")];
        ...
        var notAllowed = [...document.getElementsByClassName("sc-seat not-allowed")];
        for (item of notAllowed) {
    ...
    var createSmallTitle = function createSmallTitle(content) {
        var smallTitle = document.createElement('span');
    ...
    var createCartItemsCounter = function createCartItemsCounter(count) {
        var cartItemsCount = document.createElement('span');
        cartItemsCount.textContent = '{0}'.format(count)+' out of '+ passengersNum;
    ...
    var createLegend = function createLegend() {
        ...
        var legendTitle = createTitle('Seat map key');

        var seatsList = createLegendList();

        for (var i = 0; i < options.types.length; i += 1) {
            var description = '{0}'.format(
        ...
        seatsList.appendChild(createLegendItem('Already booked / other classes', 'unavailable'));
    ...
    var createCart = function createCart() {
        ...
        var cartTitle = createIconedTitle(
            'Seats Selected',
            '{0}/icons/seat.svg'.format(options.assets.path),
            'Seats Selected icon.'
        );
